  ...
  <aside class="w-60 shrink-0 flex flex-col bg-white border-r border-slate-100 min-h-screen">
    ...
    <nav class="flex-1 px-3 py-4 flex flex-col gap-1">
      ...
      >
        ...
        <span v-if="todos.todos.length"
          class="text-xs font-semibold px-1.5 py-0.5 rounded-full"
          :class="$route.path.startsWith('/todos') ? 'bg-primary-100 text-primary-700' : 'bg-slate-100 text-slate-500'">
          {{ todos.todos.length }}
        </span>
      </RouterLink>
    </nav>
    ...
    <div class="px-3 py-4 border-t border-slate-100 flex flex-col gap-1">
      <RouterLink to="/settings"
        class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150"
        :class="$route.path === '/settings' ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-800'"
      >
